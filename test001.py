...
#직접 가져오기
s1 = spritesheet.spritesheet('스프라이트_기본UI3.png')
...
for i in range(0,8):
    item_food.append(s1.image_at((1+32*(i),1+42+20+20+32+32+32, 30,30)))
    item_food[i]=pygame.transform.scale(item_food[i],(90,90))
for i in range(0,2):
    item_play.append(s1.image_at((1+32*(i),1+42+20+20+32+32+32+32, 30,30)))
    item_play[i]=pygame.transform.scale(item_play[i],(90,90))
for i in range(2,4):
    item_temp.append(s1.image_at((1+32*(i),1+42+20+20+32+32+32+32, 30,30)))
    item_temp[i-2]=pygame.transform.scale(item_temp[i-2],(90,90))
    
#아래쪽으로만 이동하는 패널
item_temp.append(s1.image_at((1+32*(6),1+42+20+20+32+32+32+32, 30,30)))
item_temp[2]=pygame.transform.scale(item_temp[2],(90,90))

for i in range(0,3):
    item_temp2.append(pygame.transform.scale(item_temp[i],(120,120)))
...
for i in range(0,3) : 
    slimeB.append(s2.image_at((1+slimetu2[0]*3, 1+slimetu2[1]*(i+2), slimetu[0] , slimetu[1])))
    slimeB[i+9] = pygame.transform.scale(slimeB[i+9],(slimetu[0]*3,slimetu[1]*3))
for i in range(0,3) : 
    slimeB.append(s2.image_at((1+slimetu2[0]*4, 1+slimetu2[1]*(i+2), slimetu[0] , slimetu[1])))
...
room_space = s5.image_at((456-225,1200-180,456+225,1200+177))
...
#애니메이션 작동을 위한 수치들
#mode는 0일때는 정지, 1이면 상승, -1이면 하강한다.
#게임 점프 
game_jump_animator = 0
game_jump_mode = 0
game_jump_accel = 0
game_jump_helper = 0
...
#아이템 목록
item_coin = 450
item_jem = [0,1,1,0,0,0]
item_jemS = [0,1,0,0]

#인테리어, 놀이는 획득 여부만 저장한다. 음식과 아이템은
#세부 개수도 저장한다. 
item_interiors = [1,0]

item_page = 0;
item_foods = [50,0,0,0,1,1,1,0]
item_plays = [0,1]
...
def call_back(types):
    if types == 0:
        ...
        screen.blit(room_space,(0,0))
        stair_blockP = pygame.transform.scale(stair_block[4],(gtu[1]*3,gtu[2]*3))
        screen.blit(stair_blockP,(128,247))
...
def show_degree():
    ...
    #여기서 추지에 따른 연산을 진행한다.
    deg_f = (int)(deg_full / 100 * (250-str))
    deg_e = (int)(deg_exp / 1000 * (250-str))
    deg_t = (int)((deg_temp +50)/100 * (250-str))
    ...
    #색깔과 좌표 시작점, 크기
    pygame.draw.rect(screen, [123,123,123],[75,310-40,254-str,16])
    pygame.draw.rect(screen, [240,240,240],[75,312-40,250-str,12])
    pygame.draw.rect(screen, [248,206,13],[75,312-40,deg_f,12])

    pygame.draw.rect(screen, [123,123,123],[75,310-20,254-str,16])
    pygame.draw.rect(screen, [240,240,240],[75,312-20,250-str,12])
    if -5 <= deg_temp <= 5:
        pygame.draw.rect(screen, [217,255,208],[75,312-20,250-str,12])
    elif -20 <= deg_temp < -5:
        pygame.draw.rect(screen, [196,255,253],[75,312-20,250-str,12])
    elif deg_temp < -20:
        pygame.draw.rect(screen, [40,197,237],[75,312-20,250-str,12])
    elif 20 >= deg_temp > 5 :
        pygame.draw.rect(screen, [255,224,208 ],[75,312-20,250-str,12])
    elif deg_temp > 20:
        pygame.draw.rect(screen, [231,127,74 ],[75,312-20,250-str,12])
    pygame.draw.rect(screen, [210,210,210 ],[75+deg_t,312-20-2,5,16])

    #자동 수치 감소--------------------------------------------------
    deg_temp = deg_temp - 0.1
    deg_full = deg_full - 0.1

    pygame.draw.rect(screen, [123,123,123],[75,310,254-str,16])
    pygame.draw.rect(screen, [240,240,240],[75,312,250-str,12])
    pygame.draw.rect(screen, [252,240,136],[75,312-40,deg_e,12])
...
def work_hot():
    ...
    screen.blit(item_temp2[0],(270,157))
    screen.blit(item_temp2[2],(270,157))
    deg_temp = deg_temp + 0.5
    work_on_hot = work_on_hot-1
...
def work_cold():
    ...
    mover = work_on_cold % 3
    
    screen.blit(item_temp2[1],(270,157))
    screen.blit(item_temp2[2],(270,157))
    deg_temp = deg_temp - 0.5
    work_on_cold = work_on_cold-1
...
def make_shade():
    ...
    if imps < stand:
        #어두울 때의 처리
        s = pygame.Surface((450,357))  
        s.set_alpha(stand-imps)                
        s.fill((100,100,100))           
        screen.blit(s, (0,0)) 
    elif imps >= stand:
        #밝을 때의 처리
        s = pygame.Surface((450,357))  
        s.set_alpha(imps-stand)                
        s.fill((255,255,180))           
        screen.blit(s, (0,0)) 
...
def make_anime_sl_norm(num) :
    ...
    if -5 <=deg_temp<= 5:
        ...
        if(num >=0 and num < 8):
            screen.blit (slimeB[9], (150,150))
            screen.blit (slimeE[14+16], (150,150))
        elif(num >=8 and num < 16):
            screen.blit (slimeB[10], (150,150))
            screen.blit (slimeE[15+16], (150,151))
        elif(num >=16 and num < 24):
            screen.blit (slimeB[11], (150,150))
            screen.blit (slimeE[16+16], (150,151))
        elif(num >=24 and num < 32):
            screen.blit (slimeB[10], (150,150))
            screen.blit (slimeE[15+16], (150,151))
        elif(num >=32 and num < 40):
            ...
            screen.blit (slimeB[9], (80,80))
            screen.blit (slimeE[14+16], (80,80))
    elif 20 >= deg_temp > 5:
        if(num >=0 and num < 8):
            screen.blit (slimeB[3], (150,150))
            screen.blit (slimeE[11+16], (150,150))
        elif(num >=8 and num < 16):
            screen.blit (slimeB[4], (150,150))
            screen.blit (slimeE[12+16], (150,151))
        elif(num >=16 and num < 24):
            screen.blit (slimeB[5], (150,150))
            screen.blit (slimeE[13+16], (150,151))
        elif(num >=24 and num < 32):
            screen.blit (slimeB[4], (150,150))
            screen.blit (slimeE[12+16], (150,151))
        elif(num >=32 and num < 40):
            ...
            screen.blit (slimeB[9], (80,80))
            screen.blit (slimeE[11+16], (80,80))
    elif deg_temp < -20:
        if(num >=0 and num < 8):
            screen.blit (slimeB[12], (150,150))
            screen.blit (slimeE[8+16], (150,150))
        elif(num >=8 and num < 16):
            screen.blit (slimeB[13], (150,150))
            screen.blit (slimeE[9+16], (150,151))
        elif(num >=16 and num < 24):
            screen.blit (slimeB[14], (150,150))
            screen.blit (slimeE[10+16], (150,151))
        elif(num >=24 and num < 32):
            screen.blit (slimeB[13], (150,150))
            screen.blit (slimeE[9+16], (150,151))
        elif(num >=32 and num < 40):
            ...
            screen.blit (slimeB[9], (80,80))
            screen.blit (slimeE[8+16], (80,80))
    elif deg_temp > 20:
        if(num >=0 and num < 8):
            screen.blit (slimeB[6], (150,150))
            screen.blit (slimeE[8+16], (150,150))
        elif(num >=8 and num < 16):
            screen.blit (slimeB[7], (150,150))
            screen.blit (slimeE[9+16], (150,151))
        elif(num >=16 and num < 24):
            screen.blit (slimeB[8], (150,150))
            screen.blit (slimeE[10+16], (150,151))
        elif(num >=24 and num < 32):
            screen.blit (slimeB[7], (150,150))
            screen.blit (slimeE[9+16], (150,151))
        elif(num >=32 and num < 6):
            screen.blit (slimeB[6], (150,150))
            screen.blit (slimeE[8+16], (150,150))
        else:
            screen.blit (slimeB[6], (150,150))
...
def UI_TEMP(sel):
    ...
    #UI 구성
    screen.blit (back_small, (20,225))
    for j in range(0,2):
        screen.blit (item_slot, (36-1+(90+6)*(j),225+10+6))


    for i in range(0,2):
        screen.blit(item_temp[i], (36-1+(90+6)*(i),225+10+6))
        if(item_coin < 10  ):
            screen.blit(item_nonslot,  (36-1+(90+6)*(i),225+10+6))
            

    #선택 띄우기
    if 1<= Sel_case <= 2:
        screen.blit(item_selected, (36-1+(90+6)*(Sel_case - 1),225+10+6))
    ...
    if sel == 2:
        if Sel_case > 1:
            ...
            Sel_case = 2
    if sel == 5 and function_on==1:
        if (Sel_case == 1 and work_on_hot == 0 and work_on_cold == 0):
            work_on_hot = 200
        elif (Sel_case == 2  and work_on_hot == 0 and work_on_cold == 0):
            work_on_cold = 200
        else :
            back = 0
...
def UI_PLAY(sel):
    ...
    for i in range(0,2):
        screen.blit(item_play[1-i], (36-1+(90+6)*(i),225+10+6))
        if(item_plays[1-i] == 0):
            screen.blit(item_nonslot,  (36-1+(90+6)*(i),225+10+6))
...
def UI_FOOD(sel):
    ...
    for i in range(0,8):
        if item_foods[i]!=0:
            food_argN.append(i)
            food_arg.append(item_foods[i])
            food_count = food_count + 1
        else :
            food_arg_empt.append(i)
    ...
    if item_page==0:
        ...
        if food_count < 5:
            for i in range(0,food_count):
                imp = food_argN[i]
                screen.blit(item_food[imp], (36-1+(90+6)*(i),225+10+6))
            for i in range(0,4-food_count):
                imp = food_arg_empt[i]
                screen.blit(item_food[imp], (36-1+(90+6)*(i+food_count),225+10+6))
                screen.blit (item_nonslot,  (36-1+(90+6)*(i+food_count),225+10+6))
        else :
            for i in range(0,4):
                imp = food_argN[i]
                screen.blit (item_food[imp], (36-1+(90+6)*(i),225+10+6))
        ...
        if food_count > 4:
            for i in range(0,food_count-4):
                imp = food_argN[i+food_count-1]
                screen.blit(item_food[imp], (36-1+(90+6)*(i),225+10+6))
            for i in range(0,8-food_count):
                imp = food_arg_empt[i]
                screen.blit(item_food[imp], (36-1+(90+6)*(i),225+10+6))
                screen.blit (item_nonslot,  (36-1+(90+6)*(i),225+10+6))
        else :
            for i in range(0,4):
                imp = food_arg_empt[i+(4-food_count)]
                screen.blit(item_food[imp], (36-1+(90+6)*(i+food_count-4),225+10+6))
                screen.blit (item_nonslot,  (36-1+(90+6)*(i+food_count-4),225+10+6))
    ...
    if sel == 5 and function_on==1:
        if (Sel_case == 1):
            deg_full = deg_full+10
        elif (Sel_case == 2):
            deg_full = deg_full+30
        elif (Sel_case == 3):
            deg_full = 0
        elif (Sel_case == 4):
            deg_full = deg_full+5
...
def UI_INTERIOR(sel):
    ...
    imps = pygame.transform.scale(award_recv[1],(90,90))
    screen.blit(imps, (36-1+(90+6)*(0),225+10+6))
    
    screen.blit(stair_shine[3], (36-20+(90+6)*(1),225+10+3-10))
...
while True:
    ...
    #미니게임-------------------------------------------------------------
        ...
        #줄넘기 미니게임
            ...
            screen.fill((120,125,160))
            make_anime_sl_mgjm(4,1)
            imp = 0
            animo = 0
            animo2 = 0
            #30개의 sin값
            #ropes = [0,52,104,156,208,259,310,358,407,454,500,544,588,629,669,707,743,777,809,839,866,891,913,934,951,966,978,988,994,998]
            ropes = [0,8,16,23,30,37,44,51,58,65,72,78,84,90,96,101,106,111,116,120,124,127,130,133,136,138,140,141,142,143,144]
            plus_accel = 0

            #적절한 가속도 값
            plus_accel = ((game_jump_animator)+33)/26

            if game_jump_mode == 0 :
                game_jump_mode = -1
            elif game_jump_mode ==1:
                if game_jump_animator < 20:
                    game_jump_animator = game_jump_animator + (2 + game_jump_accel)*plus_accel
                    if game_jump_animator > 20:
                        game_jump_animator = 20
                else :
                    game_jump_mode = -1
                    if game_jump_accel < 8:
                        game_jump_accel = game_jump_accel + 0.3
            elif game_jump_mode == -1:
                if game_jump_animator > -20:
                    game_jump_animator = game_jump_animator - (2 + game_jump_accel)*plus_accel
                    if game_jump_animator < -20:
                    ...
                    game_jump_mode = 1
                    '''
                    if game_jump_accel < 6:
                        game_jump_accel = game_jump_accel + 0.3
                    '''


    
            #animo는 상수 느낌의 보정값 이다.
            animo2 = (int)(game_jump_animator/2)
            #캐릭터 보정 값
            helper_anime = (int)((game_jump_animator+20)/5)
            
            
            screen.blit (rope_helper[helper_anime], (45-60,178-4+animo2-60))
            impHelper = pygame.transform.flip(rope_helper[helper_anime], True, False)
            screen.blit (impHelper, (409-55,178-4+animo2-60))

            
            sup_color = 0
            if (game_jump_mode == -1 and game_jump_animator < 0):
                sup_color = game_jump_animator
            elif (game_jump_mode == 1 and game_jump_animator < 0):
                sup_color = (game_jump_animator)*(-1)-40
            elif (game_jump_mode == 1 and game_jump_animator >= 0):
                sup_color = -40 + game_jump_animator
            elif (game_jump_mode == -1 and game_jump_animator >= 0):
                sup_color = game_jump_animator*(-1)
                
                
            
        
            
            #sup_color = game_jump_animator -20
            sup_color2 = sup_color*2
            sup_color_extra = 0
            sup_color_extra2 = 0
            ...
            for i in range (0,31):
                animo = (int)(ropes[i]*game_jump_animator/20)
                if i == 0:
                    pygame.draw.rect(screen, [240,214,81],[45+6*i-4,178+animo+animo2-4,8,8])
                    pygame.draw.rect(screen, [240,214,81],[415-6*i-6,178+animo+animo2-4,8,8])
                elif i > 0 and i < 11:
                    sup_color_extra = (int)((sup_color * i)/10)
                    sup_color_extra2 = (int)((sup_color2 * i)/10)
                    pygame.draw.rect(screen, [240+sup_color_extra2,214+sup_color_extra2,81+sup_color_extra],[45+6*i-4,178+animo+animo2-4,9,8])
                    pygame.draw.rect(screen, [240+sup_color_extra2,214+sup_color_extra2,81+sup_color_extra],[415-6*i-6,178+animo+animo2-4,9,8])
                else : 
                    pygame.draw.rect(screen, [240+sup_color2,214+sup_color2,81+sup_color],[45+6*i-4,178+animo+animo2-4,11,9])
                    pygame.draw.rect(screen, [240+sup_color2,214+sup_color2,81+sup_color],[415-6*i-6,178+animo+animo2-4,11,9])
